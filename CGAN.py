get_ipython().system('python3 -m pip install jittor')
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.label_emb = nn.Embedding(opt.n_classes, opt.n_classes)  # 用于将n_classes类整数标签转换为n_classes维向量
...
number = "13520473509"  # 写入你注册时绑定的手机号（字符串类型）
n_row = len(number)
